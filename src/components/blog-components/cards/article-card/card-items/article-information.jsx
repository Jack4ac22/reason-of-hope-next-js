...
export default function ArticleInformation({ article }) {
  return (
    <>
      <div className="flex flex-col items-start justify-center overflow-y-scroll">
            ...
            <p className="information-card-text">
              ...
              {article.authors.map((author, index) => (
                <span key={author.name + index}>
                  <Link href={author.link}>{author.name}{index < article.authors.length - 1 ? ' - ' : '.'}</Link>
                </span>
              ))}
            </p>
          </div>)
            ...
            <p className="information-card-text">
              ...
              {article.categories.map((category, index) => (
                <span key={category + index}>
                  <Link href={`/categories/${category}`}>{category.replace(/-/g, ' ')}{index < article.categories.length - 1 ? ' - ' : '.'} </Link>
                </span>
              ))
              }
            </p>
