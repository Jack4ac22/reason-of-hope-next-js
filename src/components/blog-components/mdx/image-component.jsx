...
export default function ImageMappingComponent({ objectElement }) {
  ...
  let isLargeWidth = objectElement?.alt?.toLowerCase().includes("large") || objectElement?.src.toLowerCase().includes("width=large" || (!isFullWidth
    && isLargeWidth));
  let isSmallWidth = objectElement?.alt?.toLowerCase().includes("small") || objectElement?.src.toLowerCase().includes("width=small");

  let isFloatLeft = objectElement?.src?.toLowerCase().includes("left") || objectElement?.alt?.toLowerCase().includes("left") || objectElement?.src?.toLowerCase().includes("position=left");
  let isFloatCenter = objectElement?.src?.toLowerCase().includes("position=center") || objectElement?.src?.toLowerCase().includes("center") || objectElement?.alt?.toLowerCase().includes("center");
  let isFloatRight = objectElement?.src?.toLowerCase().includes("position=right") || objectElement?.src?.toLowerCase().includes("right") || objectElement?.alt?.toLowerCase().includes("right") || objectElement?.src?.toLowerCase().includes("position=right");
