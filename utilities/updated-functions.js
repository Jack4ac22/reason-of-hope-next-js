...
export function getArticleData(articleIdentifier, articlesDirectoryPath) {
  ...
  const fullSlug = `${articleDateInformations.year}/${articleDateInformations.month}/${articleDateInformations.day}/${articleSlug}`;
  const isBook = articlesDirectoryPath == "publications" ? true : false;
  const articleData = {
    slug: articleSlug,
    redirect: articlesDirectoryPath + "/" + articleSlug,
    fullSlug: fullSlug,
...
export function allArticlesData() {
  console.log("allArticlesData started");
  const jsonFilePath = path.join(process.cwd(), "/assets/articles.json");
  // check if the file exists, its json and it has a field with the key "lastUpdate"
  if (!fs.existsSync(jsonFilePath)) {
    let articlesInJson = [];
    articlesDirectoryPath.map((directory) => {
      // console.log(directory);
      const articleFiles = getArticleFiles(`/content/${directory}`);
      ...
      // console.log("articlesInJson: ", articlesInJson.length);‰
    });
    if (articlesInJson.length > 0) {
      fs.writeFileSync(jsonFilePath, JSON.stringify(articlesInJson));
    }
    console.log("allArticlesData ended with writing to file");
    return articlesInJson;
  } else {
    // read  the file and return its content
    const fileContent = fs.readFileSync(jsonFilePath, "utf-8");
    const articlesInJson = JSON.parse(fileContent);
    console.log("allArticlesData ended with reading from file");
    return articlesInJson;
  }
}
